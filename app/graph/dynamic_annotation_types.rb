DynamicAnnotation::AnnotationType.select('annotation_type').map(&:annotation_type).each do |type|
  ...
  Object.class_eval <<-TES
    ...
    module DynamicAnnotation#{klass}Mutations
      ...
      Create, Update, Destroy = Mutations::GraphqlCrudOperations.define_crud_operations('dynamic_annotation_#{type}', create_fields, update_fields, parents) unless defined? Create
    end

    DynamicAnnotation#{klass}Type = Mutations::GraphqlCrudOperations.define_annotation_type('dynamic_annotation_#{type}', {}) do
